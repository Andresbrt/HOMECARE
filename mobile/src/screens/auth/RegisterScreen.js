...
export default function RegisterScreen({ route, navigation }) {
  ...
  const handleGoogleRegister = async () => {
    ...
    try {
      const result = await loginWithGoogle(undefined, selectedRole);
      if (!result.success && !result.cancelled) {
        Alert.alert('Error', result.message || 'Error al registrarse con Google');
      }
    } catch (err) {
      Alert.alert('Error', err.message || 'Error al registrarse con Google');
    } finally {
              ...
              {googleLoading ? (
                ...
                <View style={styles.googleBtnInner}>
                  ...
                  <Text style={styles.googleBtnText}>
                    {isProvider ? 'Registrarme con Google como Pro' : 'Registrarme con Google'}
                  </Text>
                </View>
              )}
            </TouchableOpacity>
