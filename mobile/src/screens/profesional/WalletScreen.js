...
export default function WalletScreen({ navigation }) {
  ...
  // ── Fetch wallet real desde backend ──────────────────────────────────────
  const fetchWalletData = useCallback(async () => {
    try {
      // Endpoint dedicado que calcula el saldo en el backend (no en el frontend)
      const { data: wallet } = await apiClient.get('/payments/wallet');
      const lista = Array.isArray(wallet.transacciones) ? wallet.transacciones : [];
      setPagos(lista);
      setStats(buildStats(lista));
      // Saldo calculado por el backend: disponible + retenido
      const disponible = Number(wallet.saldoDisponible ?? 0);
      const retenido   = Number(wallet.saldoRetenido ?? 0);
      setBalance(disponible);
      setRetenido(retenido);
    } catch {
