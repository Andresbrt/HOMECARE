...
export default function EditProfileScreen({ navigation }) {
  ...
  const [fotoUri, setFotoUri]           = useState(user?.fotoPerfil || null); // URL de Supabase Storage o null
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      // 2. Guardar en Backend (PostgreSQL)
      const profileData = {
        nombre: nombre.trim(),
      ...
      await apiClient.put('/usuarios/me', profileData);
