...
function ServiceCard({ item, onRate, onPress, isProfessional, onCancel }) {
    ...
    >
      ...
      {item.estado !== 'COMPLETADO' && item.estado !== 'CANCELADO' && (
        <TouchableOpacity style={styles.cancelButton} onPress={() => onCancel(item)}>
          <Ionicons name="close-circle" size={15} color={PROF.error} />
      ...
      Alert.alert(
        ...
        [
          ...
          { text: 'Sí', onPress: async () => {
              const motivo = 'Usuario canceló';
              await apiClient.delete(`/solicitudes/${service.id}`, { params: { motivo } });
              Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
