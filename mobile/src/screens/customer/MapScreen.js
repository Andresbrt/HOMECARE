 ...
 * Full-screen dark premium map estilo InDriver — Glassmorphism UI
 */
...
if (Platform.OS !== 'web') {
  ...
  MapView = ({ children, style }) => (
    <View style={[style, { backgroundColor: '#0a1628', justifyContent: 'center', alignItems: 'center' }]}>
      <Text style={{ color: '#49C0BC' }}>Mapa no disponible en Web (Usa Expo Go)</Text>
      {children}
    </View>
  );
    ...
    <View style={styles.screen}>
      ...
      <Animated.View entering={FadeInDown.duration(480).delay(80)} style={[styles.minimalHeader, { top: insets.top + 10 }]}>
        <TouchableOpacity 
          ...
          onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); setIsMenuOpen(true); }}
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.topCategoriesScroll}>
          {CATEGORIES.map(cat => (
            <CategoryChip 
              ...
              onPress={(id) => { setSelectedCategory(id); navigation.navigate('ServiceRequestFlow', { serviceType: id }); }} 
            />
          ))}
        </ScrollView>
      </View>
            ...
            <SafeAreaView style={{ flex: 1 }}>
              <TouchableOpacity style={styles.sideMenuHeader} onPress={() => { setIsMenuOpen(false); navigation.navigate('UserProfile'); }}>
                <View style={styles.sideAvatarContainer}>
                  <LinearGradient colors={PROF.gradAccent} style={styles.sideAvatar}>
                    <Text style={styles.sideAvatarText}>{initials}</Text>
                  </LinearGradient>
              ...
              <ScrollView style={styles.menuOptions}>
                {/* Servicios rápidos de limpieza */}
                <View style={styles.quickServicesHeader}>
                  <Ionicons name="flash" size={13} color="#49C0BC" />
                  <Text style={styles.quickServicesTitle}>Servicios rápidos</Text>
                </View>
                {[
