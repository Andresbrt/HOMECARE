...
import { signInWithGoogle as supabaseSignInWithGoogle } from '../services/supabaseAuthService';
  ...
  const loginWithGoogle = async (supabaseOrGoogleToken, selectedRole) => {
    try {
      ...
      if (supabaseOrGoogleToken) {
        // Verificar si es un token de sesión de Supabase existente
        const { data: sessionData } = await supabase.auth.getSession();
        if (sessionData?.session?.access_token === supabaseOrGoogleToken) {
          session = sessionData.session;
        } else {
          // Es un idToken de Google → autenticar contra Supabase
        ...
        // Iniciar flujo de Google (nativo o WebBrowser)
        session = await supabaseSignInWithGoogle();
      }
      ...
      // Si el usuario canceló el diálogo o cerró el navegador
      if (!session) {
        return { success: false, cancelled: true };
      }

      // Extraer datos del usuario
      const sbUser = session.user || (await supabase.auth.getUser()).data?.user;
      const meta = sbUser?.user_metadata || {};
      const createdAt = new Date(sbUser?.created_at || 0).getTime();
      const lastSignIn = new Date(sbUser?.last_sign_in_at || 0).getTime();
      const isNewUser = Math.abs(lastSignIn - createdAt) < 45_000;

      // Si es nuevo usuario y aún no se ha especificado rol, solicitar selección
      if (isNewUser && !selectedRole) {
        return { success: true, isNewUser: true, pendingOAuthToken: session.access_token };
      }

      const nombre = meta.given_name || meta.full_name?.split(' ')[0] || meta.name?.split(' ')[0] || 'Usuario';
