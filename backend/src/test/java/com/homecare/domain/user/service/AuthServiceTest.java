...
class AuthServiceTest {
    ...
    private AuthDTO.Registro buildRegistro(String rol) {
        return AuthDTO.Registro.builder()
                ...
                .telefono("3109876543")
                .rol(rol)
                .fotoSelfieBase64("data:image/jpeg;base64,mockbase64")
                .fotoCedulaFrontalBase64("data:image/jpeg;base64,mockbase64")
                .fotoCedulaPosteriorBase64("data:image/jpeg;base64,mockbase64")
                .archivoAntecedentesBase64("data:image/jpeg;base64,mockbase64")
                .build();
    }
    ...
    class RegistroTests {
        ...
        void registro_customer() {
            ...
            assertThat(response.getRol()).isEqualTo("CUSTOMER");
        ...
        @Test
        @DisplayName("throws when role not allowed for public registration")
        void registro_unknownRole() {
            ...
            assertThatThrownBy(() -> authService.registro(req))
                    ...
                    .hasMessageContaining("no está permitido para autoregistro");
            verify(usuarioRepository, never()).save(any());
        }

        @Test
        @DisplayName("security: rejects privilege escalation attempt to register as ADMIN")
        void registro_rejectAdminRoleInjection() {
            AuthDTO.Registro reqAdmin = buildRegistro("ADMIN");

            when(usuarioRepository.existsByEmail(reqAdmin.getEmail())).thenReturn(false);
            when(usuarioRepository.existsByTelefono(reqAdmin.getTelefono())).thenReturn(false);

            assertThatThrownBy(() -> authService.registro(reqAdmin))
                    .isInstanceOf(AuthException.class)
                    .hasMessageContaining("no está permitido para autoregistro");

            AuthDTO.Registro reqRoleAdmin = buildRegistro("ROLE_ADMIN");
            assertThatThrownBy(() -> authService.registro(reqRoleAdmin))
                    .isInstanceOf(AuthException.class)
                    .hasMessageContaining("no está permitido para autoregistro");

            verify(usuarioRepository, never()).save(any());
            verify(rolRepository, never()).findByNombre(eq("ROLE_ADMIN"));
        }
    ...
    class ObtenerInfoTests {
        ...
        void obtenerInfo_success() {
            ...
            assertThat(info.getRol()).isEqualTo("CUSTOMER");
