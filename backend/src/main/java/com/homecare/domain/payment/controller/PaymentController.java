...
public class PaymentController {
    ...
    /**
     * Wallet del proveedor: saldo disponible, retenido y total ganado.
     * El balance se calcula en el backend — el frontend no puede manipularlo.
     */
    @GetMapping("/wallet")
    @PreAuthorize("hasRole('SERVICE_PROVIDER')")
    @Operation(summary = "Obtener cartera del proveedor (saldo calculado en backend)")
    public ResponseEntity<PagoDTO.WalletResponse> getWallet(
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        PagoDTO.WalletResponse wallet = paymentService.obtenerWalletProveedor(userDetails.getId());
