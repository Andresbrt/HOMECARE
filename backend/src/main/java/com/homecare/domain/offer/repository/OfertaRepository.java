...
public interface OfertaRepository extends JpaRepository<Oferta, Long> {
    ...
    /**
     * Obtiene una oferta con bloqueo pesimista de escritura (FOR UPDATE).
     * Usado en aceptarOferta para garantizar exclusión mutua a nivel de BD.
     */
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT o FROM Oferta o WHERE o.id = :id")
    Optional<Oferta> findByIdWithLock(@Param("id") Long id);
    @org.springframework.data.jpa.repository.EntityGraph(attributePaths = {"proveedor"})
    ...
    // Ofertas del proveedor con carga ansiosa de la solicitud
    @org.springframework.data.jpa.repository.EntityGraph(attributePaths = {"solicitud", "solicitud.cliente"})
