...
public class ServicioAceptadoService {
    ...
    private final com.homecare.domain.user.repository.UsuarioRepository usuarioRepository;
    ...
    public ServicioDTO.Response actualizarEstado(Long servicioId, Long proveedorId,
        ...
        if (nuevoEstado == EstadoServicio.COMPLETADO) {
            // Actualización de ranking en backend: incrementa contador de servicios completados reales
            com.homecare.domain.user.model.Usuario prov = servicio.getProveedor();
            if (prov != null) {
                int actual = prov.getServiciosCompletados() != null ? prov.getServiciosCompletados() : 0;
                prov.setServiciosCompletados(actual + 1);
                usuarioRepository.save(prov);
                log.info("Servicios completados actualizados para proveedor {}: {}", prov.getId(), actual + 1);
            }
