package com.homecare.domain.solicitud.repository;
...
public interface SolicitudRepository extends JpaRepository<Solicitud, Long> {
    ...
    /**
     * Obtiene una solicitud con bloqueo pesimista (SELECT FOR UPDATE).
     * Previene condiciones de carrera cuando múltiples transacciones intentan aceptar ofertas
     * simultáneas para la misma solicitud.
     */
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT s FROM Solicitud s WHERE s.id = :id")
    Optional<Solicitud> findByIdWithLock(@Param("id") Long id);

    // Solicitudes del cliente con carga ansiosa de ofertas para evitar N+1
    @EntityGraph(attributePaths = {"cliente", "ofertas"})
    List<Solicitud> findByClienteIdOrderByCreatedAtDesc(Long clienteId);

    // Solicitudes abiertas (recibiendo ofertas) con carga ansiosa
    @EntityGraph(attributePaths = {"cliente", "ofertas"})
    @Query("""
