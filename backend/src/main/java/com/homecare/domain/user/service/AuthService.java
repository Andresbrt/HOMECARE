...
public class AuthService {
    ...
    private static final Set<String> ROLES_REGISTRO_PERMITIDOS = Set.of(
            "CUSTOMER", "CLIENTE", "ROLE_CUSTOMER",
            "SERVICE_PROVIDER", "PROVEEDOR", "ROLE_SERVICE_PROVIDER"
    );

    private String resolvePublicRegistrationRole(String rawRole) {
        if (rawRole == null || rawRole.isBlank()) {
            return "ROLE_CUSTOMER";
        }
        String cleanRole = rawRole.trim().toUpperCase();
        if (!ROLES_REGISTRO_PERMITIDOS.contains(cleanRole)) {
            throw new AuthException("El rol '" + rawRole + "' no está permitido para autoregistro.");
        }
        if (cleanRole.contains("PROVIDER") || cleanRole.contains("PROVEEDOR")) {
    ...
    public AuthDTO.LoginResponse registro(AuthDTO.Registro registroDTO) {
        ...
        // Blindaje contra escalación de privilegios: Solo roles públicos permitidos
        String rolNombre = resolvePublicRegistrationRole(registroDTO.getRol());
        Rol rol = rolRepository.findByNombre(rolNombre)
                .orElseThrow(() -> new AuthException("Rol no encontrado: " + rolNombre));

        boolean esProveedor = "ROLE_SERVICE_PROVIDER".equals(rolNombre);
    ...
    public AuthDTO.LoginResponse loginWithSupabaseToken(AuthDTO.SupabaseLogin dto) {
        ...
        Usuario usuario = usuarioRepository.findBySupabaseUid(supabaseUid)
                ...
                .orElseGet(() -> {
                    ...
                    // Blindaje contra inyección de rol: solo roles públicos permitidos
                    String rolNombre = resolvePublicRegistrationRole(dto.getRol());
                    Rol rol = rolRepository.findByNombre(rolNombre)
